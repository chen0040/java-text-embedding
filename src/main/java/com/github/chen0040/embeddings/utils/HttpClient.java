...
public class HttpClient implements Serializable {
   ...
   public static <T> List<T> getArray(final String url, Class<T> clazz){
    ...
    public static boolean downloadFile(final String url, final String save_to_file) {
       return downloadFile(url, save_to_file, new HashMap<>());
    }

   public static boolean downloadFile(final String url, final String save_to_file, final Map<String, String> headers){
      try {
         CloseableHttpClient httpClient = buildClient();
         HttpGet request = new HttpGet(url);
         for (Map.Entry<String, String> entry : headers.entrySet()) {
         ...
         logger.error("Failed to download the file from " + url, ex2);
         return false;
      }
   }
